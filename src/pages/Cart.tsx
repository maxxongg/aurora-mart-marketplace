      ...
      <div className="grid lg:grid-cols-3 gap-6 sm:gap-8">
          ...
          {items.map((item) => (
            <div key={item.id} className="flex gap-3 sm:gap-4 p-3 sm:p-4 rounded-lg border bg-card">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs sm:text-sm text-muted-foreground mt-0.5">{settings.currency}{item.product.price.toFixed(2)} each</p>
                <div className="flex items-center justify-between mt-2 sm:mt-3">
                  ...
                  <div className="flex items-center gap-2 sm:gap-3">
                    <span className="font-display font-bold text-sm sm:text-base">{settings.currency}{(item.product.price * item.quantity).toFixed(2)}</span>
        ...
        <div className="bg-card border rounded-lg p-5 sm:p-6 h-fit lg:sticky lg:top-24">
          ...
          <div className="flex justify-between font-display font-bold text-lg mb-6">
            <span>Total</span><span>{settings.currency}{(total + shipping).toFixed(2)}</span>
          </div>
