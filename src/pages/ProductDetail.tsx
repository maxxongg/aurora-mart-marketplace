  ...
  return (
    <div className="container mx-auto py-4 sm:py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-12">
        <div>
          <div className="aspect-square rounded-xl overflow-hidden bg-secondary mb-3 sm:mb-4">
            <img src={images[selectedImage]} alt={product.name} className="w-full h-full object-cover" />
          </div>
          {images.length > 1 && (
            <div className="flex gap-2">
              {images.map((img, i) => (
                <button key={i} onClick={() => setSelectedImage(i)} className={`h-14 w-14 sm:h-16 sm:w-16 rounded-lg overflow-hidden border-2 transition ${i === selectedImage ? "border-primary" : "border-transparent"}`}>
        ...
        <div>
          ...
          <div className="flex items-baseline gap-2 sm:gap-3 mb-4 sm:mb-6">
            <span className="font-display text-2xl sm:text-3xl font-bold">${product.price.toFixed(2)}</span>
            {product.originalPrice && <span className="text-base sm:text-lg text-muted-foreground line-through">${product.originalPrice.toFixed(2)}</span>}
          ...
          <div className="space-y-2.5 sm:space-y-3 text-sm">
            <div className="flex items-center gap-3"><Truck className="h-4 w-4 text-primary shrink-0" /> Free shipping on orders over $50</div>
      ...
      {related.length > 0 && (
        <section className="mt-12 sm:mt-16">
          <h2 className="font-display text-xl sm:text-2xl font-bold mb-4 sm:mb-6">Related Products</h2>
